...
const LifestylePopularPost = () => {
  const [allPost, setAllPost] = useState();
  const [loading, setLoading] = useState(true);
  const fetchingAllCryptoData = async (mounted ) => {
    try {
      const { data } = await axios.post("http://localhost:8080/api/category/posts/lifestyle&fashion/privatepopular");
...
export default LifestylePopularPost;
